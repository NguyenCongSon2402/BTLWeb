 ...
 * @author PC
 */
@WebServlet(name = "HistoryShopingControl", urlPatterns = {"/history"})
public class HistoryShopingControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Bill> list = bill.getBill(a.getId()); //Truyền vào id của account
        int total = 0;
        int amount = 0;
        for(Bill b : list) {
            total += b.getTotal();
        ...
        request.getRequestDispatcher("HistoryShoping.jsp").forward(request, response);
    }
